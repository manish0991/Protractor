...
def click_by_xpath(xpath)
  begin
    wait_for_element_visible(xpath, 30)
	@browser.find_element(:xpath, "#{xpath}").location_once_scrolled_into_view
    @browser.find_element(:xpath, "#{xpath}").click
...
def storeData(filename,data)
 base_path=File.expand_path(File.dirname(File.dirname(__FILE__)))
 puts base_path
    value=base_path.split("features_v2")
    file="#{value[0]}features_v2/Filestack/#{filename}.txt"
 if File.exist?(file)
  f = File.open(file, "w")
  f.write(data)
  f.close
